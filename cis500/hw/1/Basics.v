...
(** Answer: 
    For first one, it will display forall n:nat, n + 0 = n,
    for second one, it will display forall n:nat n = n. I verified it 
    by running the code.

    The difference is that for first one, n is not a "neutral element",
    so the "n + 0" cannot be simplified by directly observing the definition
    of plus, but in the second one, 0 is a "neutral element", and "0 + n"
    can be simplified by looking at the definition of plus.
*)
...
Proof.
  intros n m.
  rewrite -> plus_O_n. (*Here simpl. also works *)
  reflexivity.  Qed.
...
Theorem mult_to_plus : forall m n : nat,
  m * (S n)  = m + m * n.
Proof.
  intros m n.
  induction m as [| m'].
...
Proof.
  ...
  Case "n = S n'".
      rewrite -> mult_to_plus. rewrite -> IHn'. reflexivity. 
Qed.      
(** [] *)
...
Fixpoint bin_to_nat (n:bin) : nat :=
  match n with
    | BO => O
    | BT n' => 2 * (bin_to_nat n')
    | BOT n' => 1 + 2 * (bin_to_nat n')
  end.
(* c *)
Theorem inc_con_comm : forall b:bin,
...
Theorem con_equiv : forall n:nat,
...
Proof.
  ...
  Case "n = S n'".
      ...
      rewrite -> inc_con_comm. reflexivity.
...
Proof.
  ...
  Case "b = BT b'".
      ...
      assert (H: forall n:nat, nat_to_bin(n + n) = BT (nat_to_bin n)).
      SCase "Proof".
          intros n. induction n as [| n'].
          SSCase "0".
              simpl. admit.
      ...
      replace (BT b') with (BT (nat_to_bin (bin_to_nat b'))).
      apply H with (n:=bin_to_nat b').
      rewrite -> IHb'. reflexivity.
  Case "b = BOT b'".
      simpl. rewrite -> plus_0_r.
      assert (H: forall n:nat, inc_bin (nat_to_bin(n + n)) = BOT (nat_to_bin n)).
      SCase "Proof".
          intros n. induction n as [| n'].
          SSCase "0".
              simpl. reflexivity.
          SSCase "S n'".
              simpl. rewrite -> plus_comm. simpl. rewrite IHn'. simpl. reflexivity.
      replace (BOT b'') with (BOT (nat_to_bin (bin_to_nat b''))).
      apply H with (n:=bin_to_nat b'').
      rewrite -> IHb''. reflexivity.
Qed.
