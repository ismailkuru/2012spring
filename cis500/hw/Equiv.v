...
(* $Date: 2012-02-29 19:59:40 -0500 (Wed, 29 Feb 2012) $ *)
...
No. For X >= 2, both has the same final state, otherwise,
first one will have X=2, but second has X=3. 
...
Proof.
  ...
  Case "<-".
      ...
      SCase "not b = false".
      apply E_IfTrue. simpl in H5. apply negb_false_iff in H5.
      assumption. assumption. 
Qed.
(** [] *)
...
(** Write an informal proof of [WHILE_false].
    ...
    _Proof_: Suppose that [bequiv b BFalse]. We need to show 
    [WHILE b DO c END] is equivalent to [SKIP].
    
      - For case "->". We need to show if [WHILE b DO c END / st||st'],
      then [SKIP / st||st']. There are two cases for the [WHILE]

        - If [WHILE b DO c END] ends at this time, we know st = st',
        therefore [SKIP / st||st'] is immediate from the definition
        of [ceval].

        - If [WHILE b DO c END] continues to loop, we know [b] is
        evaluted to [true], which is contradicted to the assumption
        [b] is equivalent to [BFalse]. So this case is impossible.

     - For case "<-". We need to show if [SKIP / st||st'], then 
     [WHILE b DO c END / st||st']. 

     From the definition of [ceval], we know st = st' from the [E_Skip] 
     case. Then we need to show
     
       [WHILE b DO c END / st'||st']. 

     It follows from the definition of [ceval] from the [E_WhileEnd] case      and our assumption that [bequiv b BFalse]. []
*)
...
Proof. 
  ...
  Case "->".
      ...
      SCase "E_WhileLoop".
          apply WHILE_true_nonterm with (c:=c) (st:=st) (st':=st') in Hb.
          apply ex_falso_quodlibet. apply Hb. assumption.
  Case "<-".
...
Proof.
  ...
  Case "->".
      ...
      rewrite update_same; reflexivity.
  Case "<-".
      inversion Hc. subst. rewrite <- H. simpl.
      assert (st = update st X (st X)).
          apply functional_extensionality. intro.
          rewrite update_same; reflexivity.
...
(* Because [empty_state] and [update] does not belong to the same
   inductive type, inversion cannot derive any additional information
   from them, nor shows two constructors are not the same.*)
...
Proof.
  ...
  Case "->".
      inversion H; subst.
      SCase "b = true".
          apply E_IfTrue. rewrite <- H1. assumption. apply H2. assumption.
      SCase "b = false".
          apply E_IfFalse. rewrite <- H1. assumption. apply H3.
          assumption.
  Case "<-".
      inversion H; subst.
      SCase "b' = true".
          apply E_IfTrue. rewrite H1. assumption. apply H2. assumption.
      SCase "b' = false".
          apply E_IfFalse. rewrite H1. assumption. apply H3. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  Case "BLe".
    simpl. 
    remember (fold_constants_aexp a) as fa.
    remember (fold_constants_aexp a0) as fa0.
    replace (aeval st a) with (aeval st fa).
    replace (aeval st a0) with (aeval st fa0).
    destruct fa; destruct fa0; try reflexivity.
        simpl. destruct (ble_nat n n0); reflexivity.
    rewrite Heqfa0. rewrite <- fold_constants_aexp_sound. reflexivity.
    rewrite Heqfa. rewrite <- fold_constants_aexp_sound. reflexivity.
  Case "BNot". 
...
Proof. 
  ...
  Case "WHILE".
    assert (bequiv b (fold_constants_bexp b)).
      SCase "Proof of assertion". apply fold_constants_bexp_sound.
    remember (fold_constants_bexp b) as b'.
...
Proof.
  ...
  induction H; 
      ...
      try reflexivity.
  Case "ID".
      simpl. unfold update. apply not_eq_beq_id_false in H. rewrite H. 
      reflexivity.
Qed.
...
Proof.
  intros contra.
  apply loop_never_stops with (st:=empty_state) (st':=empty_state).
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st || st''
  | E_Havoc : forall (st st':state) (X:id) (n:nat), 
                 update st X n = st' ->
                 (HAVOC X) / st || st'
...
Proof.
  apply E_Havoc with (n:=0) (st:=empty_state) 
                     (st':=update empty_state X 0).
  reflexivity.
Qed.
...
Proof.
  ...
  apply E_Skip.
  apply E_Havoc with (n:=42) (st:=empty_state) 
                     (st':=update empty_state Z 42).
  reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  split; intros H.
  Case "->".
      inversion H. subst. inversion H2. inversion H5. subst.
      apply E_Seq with (st':=update st Y n0).
      apply E_Havoc with (n:=n0). reflexivity.
      apply E_Havoc with (n:=n). apply functional_extensionality.
      intros. rewrite update_permute. reflexivity. reflexivity.
  Case "<-".
      inversion H. subst. inversion H2. inversion H5. subst.
