...
(* $Date: 2012-02-08 17:06:54 -0500 (Wed, 08 Feb 2012) $ *)
...
Definition funny_prop1 := forall n, forall (E : beautiful n), beautiful (n+3).
...
(** If we had a proof term inhabiting this proposition, it would be a
    ...
    [n] is beautiful.  But the name [E] for this evidence is not used in
...
Definition funny_prop1' := forall n, forall (_ : beautiful n), beautiful (n+3).

(** Or, equivalently, we can write it in more familiar notation: *)

Definition funny_prop1'' := forall n, beautiful n -> beautiful (n+3).
...
(** Now we can prove the other direction of the equivalence of [even]
   and [ev], which we left hanging in [Prop.v].  Notice that the
...
Proof.
  ...
  Case "n = 0".
      ...
      SCase "Right".
          ...
          inversion H.
  Case "n = S n'".
      inversion IHn' as [HL HR].
      split.
      SCase "Left".
          apply HR.
      SCase "Right".
          intros H. unfold even in H. simpl in H.
          apply ev_SS.
          apply HL.
          unfold even. 
          apply H.
...
(** Note that some theorems that are true in classical logic are _not_
    provable in Coq's "built in" constructive logic.  E.g., let's look
    at how this proof gets stuck... *)
...
(** Proofs of facts about [<=] using the constructors [le_n] and
    ...
    [ev] in [Prop.v].  We can [apply] the constructors to prove [<=]
