...
(* $Date: 2012-04-18 19:37:42 -0400 (Wed, 18 Apr 2012) $ *)
...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tunit => tunit
  | tlet x' t1 t2 =>
    tlet x' (subst x s t1) (if beq_id x x' then t2
                            else subst x s t2)
  | tinl Tl t1 => tinl Tl (subst x s t1)
  | tinr Tr t1 => tinr Tr (subst x s t1)
  | tcase t0 x1 t1 x2 t2 =>
    tcase (subst x s t0) x1 (if beq_id x x1 then t1 else subst x s t1)
                         x2 (if beq_id x x2 then t2 else subst x s t2)
  | tnil T => tnil T
  | tcons t1 t2 => tcons (subst x s t1) (subst x s t2)
  | tlcase t1 t2 y z t3 =>
     tlcase (subst x s t1) (subst x s t2) y z 
     (if beq_id x y then t3 
      else if beq_id x z then t3
      else subst x s t3)
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Case : forall (Gamma:context) (t0 t1 t2:tm) (T T1 T2:ty) (x1 x2:id),
  ...
  | T_Lcase : forall (Gamma:context) (h t:id) (t1 t2 t3:tm) (T T1:ty),
      has_type Gamma t1 (TList T1) ->
      has_type Gamma t2 T ->
      has_type (extend (extend Gamma h T1) t (TList T1)) t3 T ->
...
Proof with eauto.
  ...
  Case "T_Pred".
    ...
    SCase "t is a value".
      inversion H; subst; try (solve by inversion).
      destruct n. exists (tnat 0). constructor.
                  exists (tnat n). constructor.
    SCase "t steps".
  ...
  Case "T_Pair".
    destruct IHHt1; subst...
    SCase "t1 is a value". destruct IHHt2; subst...
         right. inversion H0. exists (tpair t1 x)...
    SCase "t1 steps". destruct IHHt2; subst...
         inversion H. right. exists (tpair x t2)...
         inversion H. right. exists (tpair x t2)...
  Case "T_Fst".
    destruct IHHt; subst...
    SCase "t is a value". inversion H; subst; try (solve by inversion).
        right. exists (t0)... 
    SCase "t steps".
        right. inversion H. exists (tfst x)...
  Case "T_Snd".
    destruct IHHt; subst...
    SCase "t is a value". inversion H; subst; try (solve by inversion).
        right. exists t0...
    SCase "t steps". right. inversion H. exists (tsnd x)...
  Case "T_Unit".
    left. constructor.
  Case "T_Let".
    right. destruct IHHt1; subst...
    SCase "t1 steps". inversion H. exists (tlet x x0 t2)...
  Case "T_Inl".
    destruct IHHt; subst... 
    inversion H. right. exists (tinl T2 x)...
  Case "T_Inr".
    destruct IHHt; subst...
    inversion H. right. exists (tinr T1 x)...
  Case "T_Case".
    destruct IHHt1; subst...
    SCase "t0 is a value".
       inversion H; subst; try (solve by inversion).
       right. exists ([x1:=t3]t1)...
       right. exists ([x2:=t3]t2)...
    SCase "t0 steps".
       right. inversion H. exists (tcase x x1 t1 x2 t2)...
  Case "T_Nil".
    left. constructor.
  Case "T_Cons".
    destruct IHHt1; subst...
    SCase "t1 is a value". destruct IHHt2; subst...
        inversion H0. right. exists (tcons t1 x)...
    SCase "t1 steps". right. inversion H. exists (tcons x t2)...
  Case "T_Lcase".
    destruct IHHt1; subst...
    SCase "t1 is a value".
  ...
  Case "T_Fix".
    ...
    SCase "t steps".
      inversion H. right. exists (tfix x)...
Qed.
...
Inductive appears_free_in : id -> tm -> Prop :=
  ...
  | afi_succ : forall (x:id) (t1:tm) ,
      appears_free_in x t1 -> appears_free_in x (tsucc t1)
  | afi_pred : forall (x:id) (t1:tm),
      appears_free_in x t1 -> appears_free_in x (tpred t1)
   ...
   | afi_mult2 : forall x t1 t2,
      ...
      appears_free_in x (tmult t1 t2) 
  | afi_if01: forall x t0 t1 t2,
      appears_free_in x t0 ->
      appears_free_in x (tif0 t0 t1 t2)
  | afi_if02: forall x t0 t1 t2,
      appears_free_in x t1 ->
      appears_free_in x (tif0 t0 t1 t2)
  | afi_if03: forall x t0 t1 t2,
      appears_free_in x t2 ->
      appears_free_in x (tif0 t0 t1 t2)
  ...
  | afi_case0: forall x x1 x2 t0 t1 t2,
      appears_free_in x t0 ->
      appears_free_in x (tcase t0 x1 t1 x2 t2)
  | afi_case1: forall x x1 x2 t0 t1 t2,
      x1 <> x ->
      appears_free_in x t1 ->
      appears_free_in x (tcase t0 x1 t1 x2 t2)
  | afi_case2: forall x x1 x2 t0 t1 t2,
      x2 <> x ->
      appears_free_in x t2 ->
      appears_free_in x (tcase t0 x1 t1 x2 t2)
  ...
  | afi_cons2: forall x t1 t2,
      appears_free_in x t2 ->
      appears_free_in x (tcons t1 t2)
  | afi_lcase1: forall x h t t1 t2 t3,
      appears_free_in x t1 ->
      appears_free_in x (tlcase t1 t2 h t t3)
  | afi_lcase2: forall x h t t1 t2 t3,
      appears_free_in x t2 ->
      appears_free_in x (tlcase t1 t2 h t t3)
  | afi_lcase3: forall x h t t1 t2 t3,
      h <> x ->
      t <> x ->
...
Proof with eauto.
  ...
  Case "T_Pair".
    apply T_Pair...
  Case "T_Let".
    apply T_Let with T1... apply IHhas_type2. intros y Hafi.
    unfold extend. remember (beq_id x y) as e.
    destruct e... 
  Case "T_Case".
    eapply T_Case... apply IHhas_type2. intros y Hafi.
    unfold extend. remember (beq_id x1 y) as e.
    destruct e...
    apply IHhas_type3. intros y Hafi.
    unfold extend. remember (beq_id x2 y) as e.
    destruct e...
  Case "T_Cons".
    apply T_Cons...
  Case "T_Lcase".
    eapply T_Lcase... apply IHhas_type3. intros y Hafi.
    unfold extend. remember (beq_id t y) as e.
    destruct e... remember (beq_id h y) as e'.
    destruct e'...
Qed.
...
Proof with eauto.
  ...
  Case "T_Lcase".
    ...
    rewrite H6 in Hctx. rewrite H3 in Hctx. assumption.
Qed.                                            
...
Proof with eauto.
  ...
  Case "tcase".
    ...
    SCase "x = i".
      destruct e2...
      SSCase "x = i0".
        eapply T_Case... eapply context_invariance... intros y Hafi.
        unfold extend. remember (beq_id i y) as e'. destruct e'...
      ...
      SSCase "x <> i0".
        simpl. eapply T_Case... eapply context_invariance... intros y Hafi.
        unfold extend. remember (beq_id i y) as e'. destruct e'...
        apply beq_id_eq in Heqe1. subst. rewrite <- Heqe'...
        apply IHt3. eapply context_invariance... intros y Hafi.
        unfold extend. remember (beq_id i0 y) as e'. destruct e'...
        apply beq_id_eq in Heqe'. subst. rewrite <- Heqe2...
    SCase "x <> i".
      destruct e2...
      SSCase "x = i0".
        simpl. eapply T_Case... apply IHt2. eapply context_invariance...
        intros y Hafi. unfold extend. remember (beq_id i y) as e'.
        destruct e'... apply beq_id_eq in Heqe'. subst. rewrite <- Heqe1...
        eapply context_invariance... intros y Hafi. unfold extend.
        remember (beq_id i0 y) as e'. destruct e'... 
        apply beq_id_eq in Heqe2. subst. rewrite <- Heqe'...
      SSCase "x <> i0".
        simpl. eapply T_Case... apply IHt2.  eapply context_invariance...
        intros y Hafi. unfold extend. remember (beq_id i y) as e'.
  ...
  Case "tlcase".
      remember (beq_id x i) as e1. remember (beq_id x i0) as e2.
      destruct e1... eapply T_Lcase... 
      eapply context_invariance... intros y Hafi. 
      unfold extend. remember (beq_id i0 y) as e'. destruct e'...
      remember (beq_id i y) as e''. destruct e''... 
      apply beq_id_eq in Heqe1. subst. rewrite <- Heqe''...
      
      destruct e2... eapply T_Lcase... 
      eapply context_invariance... intros y Hafi. 
      unfold extend. remember (beq_id i0 y) as e'. destruct e'...
      remember (beq_id i y) as e''. destruct e''... 
      apply beq_id_eq in Heqe2. subst. rewrite <- Heqe'...
