...
(* $Date: 2012-02-02 13:24:54 -0500 (Thu, 02 Feb 2012) $ *)
...
Proof.
   (* This is the same proof we showed before using inference 
      rules. First we use the rule [b_sum], telling Coq how to
      instantiate [n] and [m]. *)
...
[[
    ...
    In view of this discussion, we might wonder whether we can write
    an expression of type [beautiful 8] by applying [b_sum] to
...
(** These proof objects are not just curiosities: they actually lie at
    the core of how Coq operates.  

    When Coq is following a proof script, what is happening internally
    is that it is gradually constructing a proof tree -- a term whose
    type is the proposition being proved.  The tactics between the
    [Proof] command and the [Qed] instruct Coq how to build up a term
...
(** At any given moment, Coq has constructed a term with some
    ...
    Thus, tactic proofs are useful and convenient because they avoid
...
Definition b_times2': forall n, beautiful n -> beautiful (2*n) :=
  (fun (n:nat) (H:beautiful n) => b_sum n (n + 0) 
                                        H (b_sum n 0 H b_0)).
...
Proof.
  ...
  apply b_0.
  intros. apply b_sum. apply H. apply H0.
Qed.  
(** [] *)
...
(** Besides _constructing_ evidence of beautifulness, we can also
    _reason about_ evidence of beautifulness.  The fact that we
    introduced [beautiful] with an [Inductive] declaration tells us
    not only that the constructors [b_0], [b_3], [b_5] and [b_sum] are
    ways to build evidence of beautifulness, but also that these two
    constructors are the _only_ ways that evidence of beautifulness
    can be built.
...
Proof.
   intros.
   (* The argument proceeds by induction on the _evidence_ H! *) 
   induction H as [|n'|n'].
   Case "g_0".
       apply b_0.
   Case "g_plus3". 
...
(** **** Exercise: 2 stars (beautiful__gorgeous) *)
...
(** In [Basics.v] we defined a _function_ [evenb] that tests a number
...
Definition double_even_pfobj: forall n:nat, ev (double n):=
  nat_ind (fun (n:nat) => ev (double n)) ev_0 
        (fun (n':nat) (H:ev (double n')) => ev_SS (double n') H).
(** [] *)
...
(** We will get stuck when we try to prove the case "n = S n'",
    because no evidence for [ev (pred n')]*)
(** [] *)
...
(** Could this proof also be carried out by induction on [n] instead
    of [E]?  If not, why not? *)
(* No. There is no available evidence for the case that [n = S n']
   when we do induction on [n]. *)
(** [] *)
...
[[
   ...
   Because for the case [n = S n'], there is no constructor in
   the definition of [ev] clearly defining the case of it, and
   we do not have any other evidence for [n = S n'] as well.
*)
(** [] *)
...
Proof.
  ...
  Case "ev_0". 
      apply H1.
  Case "ev_SS".
      apply IHH2'. simpl in H1. apply SSev_even in H1. apply H1.
Qed.
(** [] *)
...
Proof.
  intros n m p H1 H2.
  apply ev_ev_even with (n:=n+n) (m:=m+p).
  rewrite <- plus_assoc. rewrite -> plus_swap' with (m:=m).
  rewrite -> plus_assoc. apply ev_sum. apply H1. apply H2.
...
Proof.
  apply nat_ind.
  Case "0". reflexivity.
  Case "S". intros n H1. simpl. rewrite -> H1. reflexivity.
Qed.
(** [] *)
...
(** This section offers some additional details on how induction works
    ...
    skimmed on a first reading.  (We recommend ski<mming rather than
...
[[
    ...
    properties of _numbers_ that are even -- we are interested in
