...
(* $Date: 2012-04-05 13:02:28 -0400 (Thu, 05 Apr 2012) $ *)
...
(** 
                     ...
                     P (C n1) (C n2) ==> C (n1 + n2)
                      ...
                      P (C n1) t2 ==> P (C n1) t2'
*)
(** Note that we're using variable naming conventions here to lighten
    the notation: by convention, [n1] and [n2] refer only to
    constants (constructed with [C]), while [t1] and [t2] refer
    to arbitrary terms.  In the formal rules, we use explicit
    constructors to make the same distinction. *)
...
Proof.
  ...
  Case "ST_Plus1".
      step_cases (inversion Hy2) SCase.
      SCase "ST_PlusConstConst". rewrite <- H0 in Hy1. inversion Hy1.
      SCase "ST_Plus1". apply IHHy1 in H2. rewrite H2. reflexivity.
...
Proof.
  intros contra.
  unfold bool_step_prop1 in contra.
  inversion contra.
Qed.
...
Proof.
  intros contra.
  unfold bool_step_prop2 in contra.
  inversion contra.
Qed.
...
Proof.  
  ...
  Case "tm_if".
      ...
      destruct x.
      SCase "tm_true".
          exists (tm_if tm_true t2 t3). apply ST_If. assumption.
      SCase "tm_false".
          exists (tm_if tm_false t2 t3). apply ST_If. assumption.
      SCase "tm_if".
          exists (tm_if (tm_if x1 x2 x3) t2 t3). apply ST_If.
          assumption.
Qed.
(** [] *)
...
(** Since we'll want to reuse the idea of multi-step reduction many
    ...
    First, a _relation_ on a set [X] is a family of propositions
    parameterized by two elements of [X] -- that is, it is a
    proposition about a pair of elements of [X]. *)

Definition relation (X: Type) := X->X->Prop.

(** Now, given a relation [R], we define a relation [multi R] as
    follows: *)
...
(** **** Exercise: 3 stars, optional (test_multistep_5) *)
...
Theorem step__eval : forall t t' n,
     t ==> t' ->
     t' || n ->
     t || n.
Proof.
...
(** Notice that we didn't actually bother to define boolean
    values -- they aren't needed in the definition of [==>b] (why?),
    though they might be if our language were a bit larger. *)
